function findTextBetweenCriteria(inputString) {
    ...
    for (const match of matches) {
        // Extract the criteria number directly from the match group
        const [, criteriaLabel, criteriaNumber, textBeforeDash, textAfterDash, endCriteria] = match;
        
        // Convert the extracted criteria number into an integer
        const criteriaNum = parseInt(criteriaNumber, 10);
        ...
        const cleanedEndCriteria = endCriteria.trim();
        
        // Append an object with the integer criteria number instead of the label
        extractedTexts.push({
            criteriaNumber: criteriaNum,
            textBeforeDash: cleanedTextBeforeDash,
            textAfterDash: cleanedTextAfterDash,
            criteriaMet: cleanedEndCriteria === "Criteria met"
        });
    }
...
function calculateGradeWithImprovements(extractedTexts) {
    // Initialize a Map to keep track of whether each criteria was met
    const criteriaMet = new Map();
    for (let i = 1; i <= 10; i++) {
        criteriaMet.set(i, false);
    }
    const improvements = []; // Array to keep track of criteria not met that need improvement
    
    // Populate the Map based on the extractedTexts
    for (const [criteriaNumber, , , met] of extractedTexts) {
        criteriaMet.set(criteriaNumber, met);
    }
    
    // Check criteria 5 and 6 first
    if (!criteriaMet.get(5)) {
        improvements.push(5);
    }
    if (!criteriaMet.get(6)) {
        improvements.push(6);
    }
    
    // Determine grade based on criteria logic, including criteria 7
    if (improvements.length > 0) { // If either 5 or 6 (or both) are not met
        const grade = improvements.includes(5) && improvements.includes(6) ? 'F' : 'D';
        return [grade, improvements];
    } else if (!criteriaMet.get(7) && !criteriaMet.get(1)) {
        improvements.push(7);
        improvements.push(1);
        return ['C', improvements];
    } else {
        // Further checks for other criteria
        if (!criteriaMet.get(1)) {
            improvements.push(1);
            return ['B', improvements];
        } else if (!criteriaMet.get(7)) {
            improvements.push(7);
            return ['B', improvements];
        } else {
            const numNotMetIn2To4 = [2, 3, 4].filter(i => !criteriaMet.get(i)).length;
            if (numNotMetIn2To4 > 1) {
                improvements.push(...[2, 3, 4].filter(i => !criteriaMet.get(i)));
                return ['B+', improvements];
            } else if (numNotMetIn2To4 === 1) {
                improvements.push(...[2, 3, 4].filter(i => !criteriaMet.get(i)));
                return ['A-', improvements];
            } else if ([...criteriaMet.values()].slice(0, 7).every(met => met) && [...criteriaMet.values()].slice(7, 10).some(met => met)) {
                return ['A+', improvements];
            } else {
                return ['A', improvements];
            }
...
async function outFunc(essay) {
    ...
    const [grade, improvementNums] = calculateGradeWithImprovements(extractedTexts);
    const doneWell = extractedTexts.filter(entry => entry[3]);
    const improvements = improvementNums.map(num => extractedTexts[num - 1]);
    const client = new OpenAI('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');
    ...
    const response = await client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a college essay assistant. You will receive a grade for an essay, the writer's strengths, and their next steps to improve on. You must deliver this with enthusiasm and humor proportional to the grade. For example, if they receive an F, you would say, for example, 'There's some way to go, but you'll get there!'. For a C maybe 'You're on the right track, but you still need to keep going'. For an A, 'What a beautiful piece of work! You are absolutely ready to submit!'\n\n\nOutput format:\n\nGrade: \nInitial thoughts\nWhat you did well:\n- point 1\n- point 2\nWhat you could improve on:\n- point 1\n- point 2\nFinal thoughts"
            },
            {
                role: "user",
...
async function main() {
    const client = new OpenAI('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');
    const essay = prompt('Put your essay in here: ');
    ...
    try {
        ...
        const extracted = gradingScheme.findTextBetweenCriteria(output);
        const [grade, improvementNums] = gradingScheme.calculateGradeWithImprovements(extracted);
        const doneWell = extracted.filter(entry => entry[3]);
        const improvements = improvementNums.map(num => extracted[num - 1]);
        console.log(output);

        const client2 = new OpenAI('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');
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('submitBtn').addEventListener('click', async function() {
        ...
        // Mocking an async operation to mimic calling an API and receiving a response
        async function mockApiCall(text) {
            return new Promise(resolve => {
                setTimeout(() => {
                    resolve(`Received essay text: ${text}`);
                }, 1000); // Simulate API call delay
            });
        }

        if (essayText.trim() === "") {
            alert("Please enter your essay in the input box.");
            return;
        }

        // Simulating calling the OpenAI API and getting a response
        const apiResponse = await mockApiCall(essayText);

        // Displaying the mock response in the output element
        document.getElementById('output').innerText = apiResponse;
    });
});
